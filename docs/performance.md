...
- **Immediate Linter Execution**: When opening a document or switching active editors, the Linter engine executes instantaneously (bypassing debounce timeouts). This guarantees that essential diagnostics (like syntax errors and ambiguous steps) appear immediately on load, while typing changes remain efficiently debounced. If `gherkinPowerTools.linter.enabled` is set to `false`, the Linter instantly enters a completely dormant state—it bypasses `setTimeout` and AST parsing entirely, guaranteeing zero CPU overhead on file modifications.
