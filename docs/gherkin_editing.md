...
- **Syntax Error Handling:** The formatter requires a valid Gherkin AST. If your document contains syntax errors (like a missing colon on a `Scenario`), automatic formatting (like Format on Save) will be completely **silent** to prevent intrusive toast warnings from interrupting your typing flow. If you trigger the Format Document command explicitly via the Command Palette or keyboard shortcut, you will receive a single concise actionable warning.
