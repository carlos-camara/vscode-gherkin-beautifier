...
### 🚀 Added
- **Execution Termination and Timeout Semantics**: Redesigned execution termination to use a strict `ExecutionOutcome` model. Infrastructure failures (like user cancellations or process timeouts) are now correctly reported as execution errors rather than test assertion failures.
- **Process Tree Cleanup**: Implemented a robust cross-platform process tree cleanup mechanism (using process groups on POSIX and `taskkill` on Windows) to guarantee no orphaned or zombie Python processes remain when test executions are cancelled or timed out.
- **Execution Timeout**: Introduced the `gherkinPowerTools.behave.executionTimeout` setting. Users can now configure a maximum allowed duration for test runs, after which the process tree is forcefully terminated. Timeouts are automatically bypassed during debugging sessions.
