...
- **Transactional Workspace Graph**: The core semantic index (`WorkspaceGraph`) has been completely redesigned around an immutable, transactional generation model (`WorkspaceGraphGeneration`). Readers (like the Impact Analyzer and Test Explorer) now query a stable, immutable snapshot of the graph, while writers use an atomic `executeTransaction` queue that strictly isolates state, eliminates race conditions, and safely aborts stale file updates during heavy workspace I/O.
