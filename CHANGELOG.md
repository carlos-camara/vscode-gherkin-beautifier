...
### 🐛 Fixed
- **Remote Environment Compatibility**: Fixed a critical bug where the Gherkin parser failed silently on Dev Containers and Remote SSH environments. The extension now natively bundles ES Module dependencies (`@cucumber/gherkin` and `@cucumber/messages`) into the production CommonJS output. This eliminates runtime module resolution failures without ballooning the VSIX file size, and permanently restores Formatting, Gherkin Health, and Test Explorer features on remote machines.
